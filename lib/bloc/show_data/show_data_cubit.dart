...
class ShowDataCubit extends Cubit<ShowDataState> {
  ...
  void logicDeleteData(int dataId) =>
      network.logicDeleteData(dataId).then((value) => emit(ShowDataDelete()));

  void loadDataUser(int dataId) {
    network.userData(dataId).then((value) {
      creator = value.userId == Config.userId;
      emit(ShowDataLoad(
        value.name,
        value.login,
        value.password,
